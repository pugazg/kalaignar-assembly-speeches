# 8-3-1973 — சட்டமன்ற மேலவையில்

Source publication: **`இருளும் ஒளியும்`**.

This entry preserves the Legislative Council reply dated `8-3-1973` in the source `பதிப்புரை`. The neutral archival ID is `1973-03-08-financial-statement-reply`; it is not asserted to be an official printed motion title.

## Locked source range

- PDF scan pages: **41–62**
- Printed pages: **40–61**
- Opening heading: `சட்டமன்ற மேலவையில்`
- Closing: scan p.62 / printed p.61, followed by the source closing ornament
- scan p.63 is printer/imprint matter; scan p.64 is the back cover
- no third speech follows

## Current state

- Gate A: **PASS** at source-package level
- Gate B: **PASS / LOCKED**
- Gate C: **COMPLETE — C1–C3 scans 41–62 / printed pp.40–61; 22/22 pages**
- Gate C.5 historical Tamil glyph audit: **IN PROGRESS — HG1–HG2 scans 41–60 / printed pp.40–59 PASS; 20/22 pages; 1 cumulative correction; 0 unresolved**
- Gates D–H: **NOT STARTED / downstream**
- Tamil status: **Gate-C complete / not verified**
- English: **blocked until Tamil verification**

## Gate C result

Gate C first-pass transcription is **COMPLETE for all 22 Unit-2 source pages**, scans **41–62 / printed pp.40–61**.

- C1: scans 41–50 / printed pp.40–49 — COMPLETE
- C2: scans 51–60 / printed pp.50–59 — COMPLETE
- C3: scans 61–62 / printed pp.60–61 — COMPLETE
- source-page markers: **41→62 exactly once and in order**
- physical crop holds introduced in Unit 2 Gate C: **0**
- source-printed English has been retained in the Tamil source layer
- scan 62 closes the speech with `வணக்கம்` and the printed closing ornament
- scan 63 is printer/imprint matter; scan 64 is the back cover; **no third speech follows**
- Tamil is still **not verified**; historical-glyph, completeness and fidelity audits remain mandatory

Gate C.5 HG1 corrected one historical `றா` misread on scan 44: `வழக்கத்திற்கு மாறுக நடனம்` → `வழக்கத்திற்கு மாறாக நடனம்`. HG2 re-read scans 51–60 and found **0 additional historical-glyph corrections**. All 13 mandatory families have now been checked through scan 60; cumulative corrections: **1**; unresolved glyph clusters: **0**.

Exact next activity: **Gate C.5 HG3 / FINAL — scans 61–62 / printed pp.60–61**. Do not begin Gate D in the same iteration.
