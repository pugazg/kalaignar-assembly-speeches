...
The controlling scan has gutter-edge loss on scans 4–5, 10–11, 20–21, 25–27 and 34–35. A newly supplied official **Tamil Nadu Legislative Assembly Debates, 7 March 1973** volume now provides an independent primary-source witness for the same speech. **CR1 bulk-populated scans 4–5 and 10–11, removing 99 legacy literal crop-marker occurrences without contextual guessing; each page still requires its own page-level revalidation.** Remaining crop holds are scans **20–21, 25–27 and 34–35**. Scan p.13 also contains small cartoon labels that remain explicitly unresolved where the scan does not support a confident reading. Scans 14–19, 22–24, 28–33 and 36–40 introduce no comparable crop loss; printed English passages are preserved in the Tamil source layer as printed. Printed illustration/cartoon labels are represented only where the scan supports a confident reading.
...
Crop recovery is now being revalidated **page by page**. Booklet scans **4–5 and 10 / printed pp.3–4 and 9 are PASS / VERIFIED**. Scan 4 reviewed 38 legacy literal crop-marker occurrences and restored 10 direct missing fragments. Scan 5 revalidated **15 line-level source-loss positions** (its legacy count 16 includes one explanatory literal token), restored 15 official-witness-supported left-gutter fragments, preserved booklet line breaks, and corrected the earlier bulk join to the physical `தலைவர்` / `திரு. பொன்னப்ப` sequence. Scan 10 revalidated **28 line-level source-loss positions** (legacy count 29 includes one explanatory literal token), restored **12** missing right-gutter fragments, preserved all physical line breaks, and corrected bulk CR1 `நில வரியை குறைத்தோம்` to the source-supported `நில வரியைக்` / next-line `குறைத்தோம்`. These pages now have **0 source-loss markers remaining** and **0 contextual guesses**. **Exact next activity: scan 11 / printed p.10 only.** Unit 2 Gate D D1 remains paused until this recovery/revalidation sequence is complete.
