...
Crop recovery is now being revalidated **page by page**. Booklet scans **4–5 / printed pp.3–4 are PASS / VERIFIED**. Scan 4 reviewed its 38 former literal crop-marker occurrences and restored 10 direct missing fragments. Scan 5 revalidated **15 line-level source-loss positions** (the earlier count of 16 includes the literal `⟦scan-crop⟧` token in its explanatory note), restored 15 official-witness-supported left-gutter fragments, preserved every booklet line break, and corrected the earlier bulk-CR1 join before Ponnappa Nadar to the physical `தலைவர்` / `திரு. பொன்னப்ப` two-line sequence. Both pages now have **0 source-loss markers remaining** and **0 contextual guesses**. **Exact next activity: scan 10 / printed p.9 only.** Unit 2 Gate D D1 remains paused until this recovery/revalidation sequence is complete.
