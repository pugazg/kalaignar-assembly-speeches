# உரை : 7 — 14.05.1998

## தொழில்துறை பற்றி கலைஞரின் சட்டமன்ற உரைகள்

**மூலத்தில் உள்ள பேச்சாளர் குறிப்பு:** மாண்புமிகு கலைஞர் மு. கருணாநிதி  
**மூல உரை எண்:** உரை : 7  
**மூலத்தில் அச்சிடப்பட்ட தேதி:** 14.05.1998  
**காப்பக ID:** `1998-05-14-industries-debate`

`industries-debate` என்பது தொகுப்பின் தொழில்துறைச் சார்பை அடிப்படையாகக் கொண்ட நடுநிலையான காப்பக slug மட்டுமே. இதை மூலத்தில் அச்சிடப்பட்ட அதிகாரப்பூர்வ சட்டமன்ற நிகழ்வு/தீர்மானத் தலைப்பாகக் கருதவில்லை.

## மூல வெளியீடு

- **நூல்:** `தொழில்துறை பற்றி கலைஞரின் சட்டமன்ற உரைகள்`
- **முதற்பதிப்பு:** மே, 2007
- **வெளியீட்டாளர்:** தமிழ்க்கனி பதிப்பகம், சென்னை - 600 004
- **விற்பனை உரிமை:** பூம்புகார் பதிப்பகம்
- **முழு controlling PDF scan pages:** 329
- **File size:** 217,124,211 bytes
- **SHA-256:** `c26003fe77b97adc6487ba0e4c00c9fa34a0a53839aa326d3ef5897f8616d370`

## இந்த உரையின் முழுப் பக்க வரம்பு

| வகை | வரம்பு |
|---|---:|
| PDF scan pages | 199–240 |
| Printed pages | 198–239 |

இந்த உரையில் scan page = printed page + 1.

## Current status

Speech 7 Tamil has completed **Gate C, Gate D and Gate E** and is **verified against scan pp.199–240**.

- Gate C: **complete — 42/42 first-pass Tamil pages**.
- Gate D completeness/page-marker audit: **passed**.
- Gate E: **passed — 42/42 pages verified against scan**.
- Gate E cumulative corrections: **5**.
- Gate E unresolved/`[REVIEW]` readings: **0**.
- English Gate F: **in progress — source/scan pp.199–223 translated (25/42 pages)**.
- English Gate G: **not started**.
- English verified: **no**.

## Gate E — canonical corrections

1. p.202 `விற்கப்படுகின்ற` → `விற்கப்படுகிற`;
2. p.205 `தெரிவித்து உண்மை` → `தெரிவித்தது உண்மை`;
3. p.209 `சுட்டிக் காட்டியிருக்கின்றேன்` → `சுட்டிக் காட்டியிருக்கிறேன்`;
4. p.214 Hyundai land allotment `552 ஏக்கர்` → `532 ஏக்கர்`;
5. p.227 `புயூஜிகுரா லிமிடெட்` → `ப்யூஜிகுரா லிமிடெட்`.

The full Gate-E closure reconciled all three visual-audit batches, confirmed exact source-page coverage 199–240, zero unresolved readings, no p.241/Speech-8 spillover, and intact opening/closing parliamentary sequences.

## Gate F — English translation progress

English is translated only from the **final verified Tamil** in [`translation.md`](./translation.md), with source-page correspondence preserved.

- Batch 1: source pp.199–203 — complete.
- Batch 2: source pp.204–208 — complete.
- Batch 3: source pp.209–213 — complete.
- Batch 4: source pp.214–218 — complete.
- Batch 5: source pp.219–223 — complete.
- Current total: **25/42 pages**.
- Next source page: **224**.
- Unresolved translation questions: **0**.

Batch 5 preserves the p.218→219 FDI-benefits continuation, the Salem integrated-steel proposal, the source’s full list of 20/15 short-term power projects and printed company forms, the Kumaramangalam reference, the SIPCOT agreements, Hyundai chronology and source figures, Integrated Iron and Steel Plant / Rynex International, Fenner Refineries Limited, the Nanguneri Hi-Tech Technology Industrial Park material, and Mahindra Industries Park Limited through the p.223 boundary. No outside factual or company-name correction was substituted.

## Next activity

Continue **Gate F Batch 6** from verified Tamil **source/scan p.224**, using a bounded five-page batch **pp.224–228 / printed pp.223–227**. Preserve exact page correspondence, figures, technical names/terms, source claims and printed English. Do not mark English verified; Gate G remains separate after the complete first pass.

Do not begin Speech 8.

## கோப்புகள்

- [`transcript.md`](./transcript.md) — complete Tamil source layer, verified against scan pp.199–240.
- [`translation.md`](./translation.md) — Gate-F English working translation, currently pp.199–223.
- [`metadata.json`](./metadata.json) — Tamil verified; Gate F in progress at 25/42 pages.
- [`source-notes.md`](./source-notes.md) — source authority, locked boundaries and verification record.
- [`verification-log.md`](./verification-log.md) — Gate-C/D history and Gate-E audit/closure trail.
